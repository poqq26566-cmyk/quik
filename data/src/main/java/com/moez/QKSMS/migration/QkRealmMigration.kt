...
class QkRealmMigration @Inject constructor(
    ...
    override fun migrate(realm: DynamicRealm, oldVersion: Long, newVersion: Long) {
        ...
        if (version == 15L) {
            val conversationSchema = realm.schema.get("Conversation")

            if (conversationSchema?.hasField("isNotification") == false) {
                conversationSchema.addField("isNotification", Boolean::class.java, FieldAttribute.REQUIRED, FieldAttribute.INDEXED)
            }
            if (conversationSchema?.hasField("notificationManuallySet") == false) {
                conversationSchema.addField("notificationManuallySet", Boolean::class.java, FieldAttribute.REQUIRED)
            }

            // Best-effort classification of existing conversations, based on the
            // first recipient's address and the last message's body, so people
            // don't have to wait for new messages to see their history sorted.
            realm.schema.get("Conversation")?.transform { conversation ->
                val recipients = conversation.getList("recipients")
                val firstAddress = if (recipients.isNotEmpty()) {
                    recipients[0]?.getString("address") ?: ""
                } else {
                    ""
                }
                val body = try {
                    conversation.getObject("lastMessage")?.getString("body") ?: ""
                } catch (e: Exception) {
                    ""
                }
                ...
                conversation.setBoolean(
                    "isNotification",
                    dev.octoshrimpy.quik.util.NotificationClassifier.isNotification(firstAddress, body)
                )
                conversation.setBoolean("notificationManuallySet", false)
            }

...
class ConversationRepositoryImpl @Inject constructor(
    ...
    private fun getConversationsBase(
        ...
        return realm
            ...
            .equalTo("isNotification", notifications)
    ...
    override fun getConversations(
        ...
        getConversationsBase(Realm.getDefaultInstance(), unreadAtTop, archived, notifications = false)
            .findAllAsync()

    override fun getConversationsSnapshot(unreadAtTop: Boolean): List<Conversation> =
        Realm.getDefaultInstance().use { realm ->
            getConversationsBase(realm, unreadAtTop, false, notifications = false)
                .findAll()
                .let(realm::copyFromRealm)
        }

    override fun getNotificationConversations(unreadAtTop: Boolean): RealmResults<Conversation> =
        getConversationsBase(Realm.getDefaultInstance(), unreadAtTop, archived = false, notifications = true)
            .findAllAsync()

    override fun getNotificationConversationsSnapshot(unreadAtTop: Boolean): List<Conversation> =
        Realm.getDefaultInstance().use { realm ->
            getConversationsBase(realm, unreadAtTop, archived = false, notifications = true)
                .findAll()
                .let(realm::copyFromRealm)
        }
    ...
    override fun markNotification(vararg threadIds: Long) =
        Realm.getDefaultInstance().use { realm ->
            val conversations = realm.where(Conversation::class.java)
                .anyOf("id", threadIds)
                .findAll()
            ...
            realm.executeTransaction {
                conversations.forEach { conversation ->
                    ...
                    conversation.notificationManuallySet = true
                }
            }
        }

    override fun markMessage(vararg threadIds: Long) =
        Realm.getDefaultInstance().use { realm ->
            val conversations = realm.where(Conversation::class.java)
                .anyOf("id", threadIds)
                .findAll()
    ...
    override fun autoClassifyNotification(threadId: Long, address: String, body: String) =
        Realm.getDefaultInstance().use { realm ->
            val conversation = realm.where(Conversation::class.java)
                .equalTo("id", threadId)
                .findFirst()

            // never override a category the user picked manually
            if (conversation != null && !conversation.notificationManuallySet) {
                val classifiedAsNotification =
                    dev.octoshrimpy.quik.util.NotificationClassifier.isNotification(address, body)

                if (conversation.isNotification != classifiedAsNotification) {
                    realm.executeTransaction { conversation.isNotification = classifiedAsNotification }
                }
            }
        }

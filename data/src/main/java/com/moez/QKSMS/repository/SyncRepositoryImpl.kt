    ...
    override fun syncMessages() {
        ...
        Handler(handlerThread.looper).post {
            Realm.getDefaultInstance().executeTransactionAsync(
                ...
                // Classify newly-synced (and existing, non-manually-set) conversations
                // as "Messages" or "Notifications", now that recipient addresses have
                // been fully populated above.
                realm.where(Conversation::class.java)
                    .equalTo("notificationManuallySet", false)
                    .findAll()
                    .forEach { conversation ->
                        val address = conversation.recipients.firstOrNull()?.address.orEmpty()
                        val body = conversation.lastMessage?.body.orEmpty()

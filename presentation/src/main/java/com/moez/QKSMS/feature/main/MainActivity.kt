...
class MainActivity : QkThemedActivity(), MainView {
    ...
    private val bottomNavIntent: Subject<NavItem> = PublishSubject.create()
...
//    override val plusBannerIntent by lazy { plusBanner.clicks() }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // 顶部工具栏的状态栏避让已经在公共基类 QkActivity 里统一处理了。
        // 这里只需要单独处理底部 Tab 栏和侧边抽屉菜单，因为只有主界面才有这两个控件。
        ViewCompat.setOnApplyWindowInsetsListener(binding.bottomNav) { view, insets ->
            val navBarInset = insets.getInsets(WindowInsetsCompat.Type.navigationBars()).bottom
            view.updatePadding(bottom = navBarInset)
            insets
        }
        val drawerContent = binding.drawer.root.findViewById<View>(R.id.drawerScrollView) ?: binding.drawer.root
        val drawerOriginalPaddingTop = drawerContent.paddingTop
        ViewCompat.setOnApplyWindowInsetsListener(drawerContent) { view, insets ->
            val statusBarInset = insets.getInsets(WindowInsetsCompat.Type.statusBars()).top
            view.updatePadding(top = drawerOriginalPaddingTop + statusBarInset)
            insets
        }
        ...
        binding.bottomNav.setOnNavigationItemSelectedListener { item ->
            val navItem = if (item.itemId == R.id.nav_notifications) NavItem.NOTIFICATIONS else NavItem.INBOX
            bottomNavIntent.onNext(navItem)
            true
        }
    ...
    override fun render(state: MainState) {
        ...
        val addContact = when (state.page) {
            ...
            is Notifications -> state.page.addContact
            else -> false
        }
        ...
        val markPinned = when (state.page) {
            ...
            is Notifications -> state.page.markPinned
            else -> true
        }
        ...
        val markRead = when (state.page) {
            ...
            is Notifications -> state.page.markRead
            else -> true
        }
        ...
        val selectedConversations = when (state.page) {
            ...
            is Notifications -> state.page.selected
            else -> 0
        }
        ...
        binding.toolbar.menu.apply {
            ...
            findItem(R.id.move_to_notifications)?.isVisible =
                state.page is Inbox && selectedConversations != 0
            findItem(R.id.move_to_messages)?.isVisible =
                state.page is Notifications && selectedConversations != 0
...
//        plus.isVisible = state.upgraded
        ...
        binding.compose.setVisible(state.page is Inbox || state.page is Archived || state.page is Notifications)
        conversationsAdapter.emptyView = binding.empty.takeIf {
            state.page is Inbox || state.page is Archived || state.page is Notifications
        }
        ...
        when (state.page) {
            ...
            is Notifications -> {
                ...
                title = when (state.page.selected != 0) {
                    true -> getString(R.string.main_title_selected, state.page.selected)
                    false -> getString(R.string.title_notifications)
                }
                if (binding.recyclerView.adapter !== conversationsAdapter)
                    binding.recyclerView.adapter = conversationsAdapter
                conversationsAdapter.updateData(state.page.data)
                itemTouchHelper.attachToRecyclerView(null)
                binding.empty.setText(R.string.notifications_empty_text)
            }
